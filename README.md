...
Two more reference pages support the second track: `using-guide.html` decodes which of the four ways of using a pretrained model (prompt, retrieve, fine-tune, train) fits a gap, and prices a month of requests; `model-cards.html` gives three specimen model cards for models built in the course, with their measured figures.
